...
void SphereObject::render(cv::Mat &outputImage) {
    ...
    if(locatingPointsProjected[0][2] > 0) {
        ...
        cv::Size2f siz(radiusProjected / cv::norm(lastCamera->baseX) * lastCamera->pixelsX / 2.0,
                       radiusProjected / cv::norm(lastCamera->baseY) * lastCamera->pixelsY / 2.0);
...
double SphereObject::getDepth(double x, double y) const {
    ...
    double coeff = 2.0 * locatingPointsProjected[0][2];
